...
For $m_0$ to be the median of a set $x$, half of the values in $x$ must be above $m_0$ and half below $m_0$. This leads to us being able to describe each value in $x$ as a Bernoulli distribution where $p=0.5$, with True meaning it is above x and False meaning below. Thus from the question we can derive the equation where $m$ is the critial value and we also see that T is binomally distributed.
...
print(m_values)
```
As we can see from this table, the critial value $m$ is the 12th value in $x$ as it is the smallest value that falls in the critical region of size 	$\alpha = 0.05$ 
...
From this we can define the two confidance intervals:

$$C_1 < m < C_2$$

$$C_1 \textrm{ is the smallest value of } m\textrm{ such that }P(T \leq m) \leq \frac{\alpha}{2}$$

$$C_2 \textrm{ is the greatest value of } m\textrm{ such that }P(T \geq m) \leq \frac{\alpha}{2}$$
```{r}
