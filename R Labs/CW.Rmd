...
$$C_1 \textrm{ is the greatest value of } x_i\textrm{ such that }P(T \leq i) \leq \frac{\alpha}{2}$$

$$C_2 \textrm{ is the smallest value of } x_i\textrm{ such that }P(T \geq i) \leq \frac{\alpha}{2}$$
```{r}
...
```
We can see that the function return the interval $0.1 \leq m < 2.8$. From question 2, if $m$ is the 12th value or more in $x$ the null hypotheses is rejected.$x_{12} = 2.8$ so this verifys that the function is correct.
