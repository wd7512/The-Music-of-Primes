...
def evolve(numberofbrains,brain):
    ...
    for i in range(numberofbrains-1-extras):
        add1=emptymatrix(13,9)
        for i in range(9):
            for j in range(13):
                add1[i,j]=random.randint(-5,5)
        add2=emptymatrix(2,13)
        for i in range(13):
            for j in range(2):
...
def play(rounds,popsize,matrixsize):
    ...
    #plt.show()
    plt.savefig('single-rounds-'+str(rounds)+'popsize-'+str(popsize)+'matrixsize-'+str(matrixsize))
    plt.cla()
    plt.clf()
    plt.imshow(bestoutput)
    plt.savefig('singlebest-rounds-'+str(rounds)+'popsize-'+str(popsize)+'matrixsize-'+str(matrixsize))
    plt.cla()
    plt.clf()
    ...
    plt.legend()
    plt.savefig('singlegraph-rounds-'+str(rounds)+'popsize-'+str(popsize)+'matrixsize-'+str(matrixsize))

    f=open('singlebest-rounds-'+str(rounds)+'popsize-'+str(popsize)+'matrixsize-'+str(matrixsize)+'.txt','a')
