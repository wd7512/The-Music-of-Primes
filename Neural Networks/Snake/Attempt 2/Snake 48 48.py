...
def random_brain():
    ...
    add1 = np.random.uniform(-1,1,(1,x))
    
    lay2 = np.random.uniform(-1,1,(x,y))
    add2 = np.random.uniform(-1,1,(1,y))

    lay3 = np.random.uniform(-1,1,(y,4))

    return [lay1,add1,lay2,add2,lay3]

def run_brain(brain,inputs):
    midlay1 = function(np.matmul(inputs,brain[0])) + brain[1]
    midlay2 = function(np.matmul(midlay1,brain[2])) + brain[3]
    output = function(np.matmul(midlay2,brain[4]))
...
def averaged_sim(pop,gens): #runs each brain x times and takes an average
    ...
    for i in range(gens):
        ...
        all_score = int(sum([B[0] for B in pop_games]) / pop)
        print('\n\nPop. Average: '+str(all_score))
        top_score = int(sum([B[0] for B in pop_games[:top_nums]]) / top_nums)
        print('Top Average: '+str(top_score))
        child_score = int(sum([B[0] for B in pop_games[-child_num:]]) / child_num)
        print('Child Average: '+str(child_score))
...
def save_brain(brain,name):
    ...
    save_matrix(brain[4],name+'_lay3')
